...
//import com.elcom.metacen.dispatcher.process.service.DispatcherProcessService;
...
//import com.elcom.metacen.dispatcher.process.utils.geo.AreaPointsDTO;
//import com.elcom.metacen.dispatcher.process.utils.geo.Point;
...
// */
//@Component
//public class SatelliteImageDataTaskLauncher_08112022 implements CommandLineRunner {
//
//    private static final Logger LOGGER = LoggerFactory.getLogger(SatelliteImageDataTaskLauncher_08112022.class);
//    
...
//        try {
//            
//            if( satelliteImageMessageRetry.getRetryNum() == Constant.MAX_RETRY_NUMBER ) {
...
//            LOGGER.info("BEGIN take SatelliteImage records from redis......");
//            while (true) {
...
//
//                                            // Tạo danh sách vùng với dữ liệu là các điểm của Vùng cấu hình
//                                            List<AreaPointsDTO> arrLst = new ArrayList<>();
//                                            String[] arr = coordinates.trim().split(",");
//                                            for( String s : arr ) {
//                                                String[] arr2 = s.trim().split(" ");
//                                                arrLst.add(new AreaPointsDTO(Double.parseDouble(arr2[0]), Double.parseDouble(arr2[1])));
//                                            }
//
//                                            // Check xem Vùng cấu hình có chứa các điểm của Vùng cắt ảnh hay không
//                                            /*
//                                                Từ  2 điểm:                                         19.2015, 114.1885    20.4095, 115.1475
//                                                ==> suy ra 2 điểm còn lại theo hình vuông/chữ nhật: 19.2015, 115.1475    20.4095, 114.1885
//                                            */
//
//                                            if( PointUtil.containsPoint2(arrLst, new Point(satelliteImageMessage.getOriginLongitude(), satelliteImageMessage.getOriginLatitude())) ) {
//                                                pass = true;
//                                                break;
//                                            }
//                                            if( PointUtil.containsPoint2(arrLst, new Point(satelliteImageMessage.getCornerLongitude(), satelliteImageMessage.getCornerLatitude())) ) {
//                                                pass = true;
//                                                break;
//                                            }
//                                            if( PointUtil.containsPoint2(arrLst, new Point(satelliteImageMessage.getCornerLongitude(), satelliteImageMessage.getOriginLatitude())) ) {
//                                                pass = true;
//                                                break;
//                                            }
//                                            if( PointUtil.containsPoint2(arrLst, new Point(satelliteImageMessage.getOriginLongitude(), satelliteImageMessage.getCornerLatitude())) ) {
//                                                pass = true;
//                                                break;
//                                            }
//                                        }
//                                    }
//                                    if( !pass ) {
//                                        // Nếu chưa thỏa mãn Rule thì cần check tiếp trường hợp Vùng cấu hình có nằm trong vùng cắt ảnh vệ tinh không ( Vùng cấu hình nhỏ hơn vùng cắt ảnh ( 100km x 100km ) )
//                                        // LOGGER.info("satellite image raw [ {} ] not match rule (1), continue checking ...", satelliteImageMessage.getUuidKey());
//
//                                        for( DataProcessConfig dataProcessConfig : dataProcessConfigs2 ) {
//                                            String coordinates = dataProcessConfig.getDetailConfig().getCoordinates();
//                                            if( !StringUtil.isNullOrEmpty(coordinates) ) {
//
//                                                // Lấy ra điểm đầu tiên và điểm thứ 2 trong Vùng cấu hình để check
//                                                String[] arr = coordinates.trim().split(",");
//
//                                                String[] arrCheck1 = arr[0].trim().split(" ");
//                                                String[] arrCheck2 = arr[1].trim().split(" ");
//
//                                                List<Point> pointToChecks = new ArrayList<>();
//                                                pointToChecks.add(new Point(Double.parseDouble(arrCheck1[0]), Double.parseDouble(arrCheck1[1])));
//                                                pointToChecks.add(new Point(Double.parseDouble(arrCheck2[0]), Double.parseDouble(arrCheck2[1])));
//
//                                                if( PointUtil.pointInsideAreaWithTwoPoints(satelliteImageMessage.getOriginLongitude()
//                                                                                         , satelliteImageMessage.getOriginLatitude()
//                                                                                         , satelliteImageMessage.getCornerLongitude()
//                                                                                         , satelliteImageMessage.getCornerLatitude()
//                                                                                         , pointToChecks) ) {
//
//                                                    pass = true;
//                                                    break;
...
//                                        if( satelliteImageMessage.getRetryNum() == 0 ) {
//
//                                        }
//
//                                    }
...
//    
//    @Bean
//    public TaskScheduler satelliteImageTaskScheduler2() {
