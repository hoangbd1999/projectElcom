...
public class Neo4jRpcServer {
    ...
    @RabbitListener(queues = "${link.object.rpc.queue}")
        ...
        try {
            ...
            if (request != null) {
                ...
                switch (request.getRequestMethod()) {
                    ...
                    case "GET":
                        if("/v1.0/link-object/topology".equalsIgnoreCase(requestPath)) // Tìm kiếm chi tiết một media cụ thể
                            response = topologyController.getTopology( headerParam,request.getUrlParam());
                        else if("/v1.0/link-object/topology/node".equalsIgnoreCase(requestPath)) // Tìm kiếm chi tiết một media cụ thể
                            response = topologyController.getTopologyNode( headerParam,request.getUrlParam());
                        break;
                        
                    case "POST":
                        if("/v1.0/link-object/node".equalsIgnoreCase(requestPath)) // Tìm kiếm danh sách media
                            response = topologyController.addNode(bodyParam,headerParam);
                        else if("/v1.0/link-object/graph/deep".equalsIgnoreCase(requestPath)) // Tìm kiếm chi tiết một media cụ thể
                            response = topologyController.getGraphDeep(bodyParam, headerParam);
                        else if("/v1.0/link-object/graph/contains".equalsIgnoreCase(requestPath)) // Tìm kiếm chi tiết một media cụ thể
                            response = topologyController.getGraphContains(bodyParam, headerParam);
                        else if("/v1.0/link-object/topology".equalsIgnoreCase(requestPath)) // Tìm kiếm chi tiết một media cụ thể
