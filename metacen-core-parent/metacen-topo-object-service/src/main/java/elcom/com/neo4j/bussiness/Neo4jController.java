...
//                    String queryDelete = "MATCH (j:Object)-[r:MEDIA]->(i:Object) where (r.dest = '"+objectInfoUpadte.getId()+"' or r.src = '"+objectInfoUpadte.getId()+"') and r.endTime >='"+ startTime +"' and r.endTime <= '"+endTime+"' delete r";
    ...
    public ResponseMessage getGraphDeep(Map<String, java.lang.Object> bodyParam, Map<String, String> headerMap) throws IOException, ParseException {
        ...
        String endTime = (String) bodyParam.get("endTime");
        String ip = (String) bodyParam.get("ip");
        List<String> typeObject = (List<String>) bodyParam.get("typeObject");
        List<Integer> typeData = (List<Integer>) bodyParam.get("typeData");
        List<Integer> dataSource = (List<Integer>) bodyParam.get("dataSource");
        String search= (String) bodyParam.get("search");
        String ids = (String) bodyParam.get("ids");
        Integer page = (Integer) bodyParam.get("page");



        return topoVsat(startTime,endTime,ip,typeObject,typeData,dataSource,search,ids,page);
    }

    public ResponseMessage getGraphContains(Map<String, java.lang.Object> bodyParam, Map<String, String> headerMap) throws IOException, ParseException {
        AuthorizationResponseDTO userInfo = null;
        userInfo = authenToken(headerMap);
        if (userInfo == null)
            return new ResponseMessage(HttpStatus.FORBIDDEN.value(), HttpStatus.FORBIDDEN.getReasonPhrase(),
        ...
        String ids = (String) bodyParam.get("ids");
        Integer page = (Integer) bodyParam.get("page");
        return topoLinkContains(startTime,endTime,typeObject,search,ids,page);
    }
...
//        headers.setBearerAuth("bmVvNGo6ZWxjb21AMTIz");
    ...
    private ResponseMessage topoVsat(String startTime,String endTime,String ip, List<String> typeObject , List<Integer> typeData,List<Integer> dataSource,String
            search,String ids, Integer page) throws ParseException {
        if(page==null){
            page=0;
        }
        ResponseLinkTopo result = topoVsatService.getLinkObject(startTime,endTime,search,ip,typeObject,typeData,dataSource,ids,page);
        return new ResponseMessage(new MessageContent(HttpStatus.OK.value(),HttpStatus.OK.toString(),result));
    }

    private ResponseMessage topoLinkContains(String startTime,String endTime, List<String> typeObject , String
            search,String ids, Integer page) throws ParseException {
        if(page==null){
            page=0;
        }
        ResponseLinkContainsTopo result = topoVsatService.getLinkContainsObject(startTime,endTime,search,typeObject,ids,page);
        return new ResponseMessage(new MessageContent(HttpStatus.OK.value(),HttpStatus.OK.toString(),result));
    }
