...
public class MetaCenMediaServiceImpl implements MetaCenMediaService {
    ...
    @Override
    public List<VsatMedia> mediaUuid(String uuid) {
        List<VsatMedia> result= vsatMediaCHRepository.searchUUid(uuid);
        if(result!=null&&!result.isEmpty()){
            List<VsatMedia> vsatMediaList= vsatMediaCHRepository.searchProcess(uuid);
            if(vsatMediaList!=null&&!vsatMediaList.isEmpty()){
                VsatMedia vsatMedia = result.get(0);
                vsatMedia.setProcessTime(vsatMediaList.get(0).getIngestTime());
                result.set(0,vsatMedia);
