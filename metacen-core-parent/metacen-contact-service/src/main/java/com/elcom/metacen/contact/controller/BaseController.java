...
import com.elcom.metacen.contact.model.dto.RoleMenuDTO;
...
public class BaseController {
    ...
    public void callLinkObjectUpdateNote( Map<String, Object> body) {
        RequestMessage abacRpcRequest = new RequestMessage();
        abacRpcRequest.setBodyParam(body);
        rabbitMQClient.callWorkerService(RabbitMQProperties.LINK_OBJECT_UPDATE_NOTE_URL, abacRpcRequest.toJsonString());
    }

    public void callLinkObjectContains( Map<String, Object> body) {
        RequestMessage abacRpcRequest = new RequestMessage();
        abacRpcRequest.setBodyParam(body);
        rabbitMQClient.callWorkerService(RabbitMQProperties.LINK_OBJECT_WORKER_QUEUE_CONTAINS, abacRpcRequest.toJsonString());
    }

    public void callLinkObjectDeleteNode( Map<String, Object> body) {
        RequestMessage abacRpcRequest = new RequestMessage();
        abacRpcRequest.setBodyParam(body);
        rabbitMQClient.callWorkerService(RabbitMQProperties.LINK_OBJECT_WORKER_QUEUE_DELETE_NODE, abacRpcRequest.toJsonString());
    }

    public ResponseMessage ChangeNameMapping(Map<String, Object> bodyParam) {
        RequestMessage mappingRpcRequest = new RequestMessage();
        mappingRpcRequest.setRequestMethod("PUT");
        mappingRpcRequest.setRequestPath(RabbitMQProperties.MAPPING_DATA_VSAT_METACEN_CHANGE_NAME_URL);
        mappingRpcRequest.setVersion(ResourcePath.VERSION);
        mappingRpcRequest.setUrlParam(null);
        mappingRpcRequest.setBodyParam(bodyParam);
        String result = rabbitMQClient.callRpcService(RabbitMQProperties.MAPPING_DATA_RPC_EXCHANGE,
                RabbitMQProperties.MAPPING_DATA_RPC_QUEUE, RabbitMQProperties.MAPPING_DATA_RPC_KEY, mappingRpcRequest.toJsonString());
    ...
    public ResponseMessage ChangeNameVsatMediaDataObjectAnalyzed(Map<String, Object> bodyParam) {
        RequestMessage mappingRpcRequest = new RequestMessage();
        mappingRpcRequest.setRequestMethod("PUT");
        mappingRpcRequest.setRequestPath(RabbitMQProperties.ENRICH_DATA_VSAT_METACEN_CHANGE_NAME_URL);
        mappingRpcRequest.setVersion(ResourcePath.VERSION);
        mappingRpcRequest.setUrlParam(null);
        mappingRpcRequest.setBodyParam(bodyParam);
        String result = rabbitMQClient.callRpcService(RabbitMQProperties.ENRICH_DATA_RPC_EXCHANGE,
                RabbitMQProperties.ENRICH_DATA_RPC_QUEUE, RabbitMQProperties.ENRICH_DATA_RPC_KEY, mappingRpcRequest.toJsonString());
        LOGGER.info("vsat media object - result: " + result);
        if (result != null) {
            return new ResponseMessage(new MessageContent(HttpStatus.OK.value(), "Change success", result));
        } else {
            return new ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(), HttpStatus.INTERNAL_SERVER_ERROR.toString(),
