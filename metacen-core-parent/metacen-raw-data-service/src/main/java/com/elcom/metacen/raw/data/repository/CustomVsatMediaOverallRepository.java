...
import com.elcom.metacen.raw.data.model.dto.ConvertSizeDTO;
...
@Repository
public class CustomVsatMediaOverallRepository extends BaseRepository {

    private static final Logger LOGGER = LoggerFactory.getLogger(CustomVsatMediaOverallRepository.class);
    ...
    @Autowired
    public CustomVsatMediaOverallRepository(@Qualifier("clickHouseSession") EntityManagerFactory factory, @Qualifier("chDatasource") DataSource dataSource) {
        super(factory, dataSource);
    }

    public Page<VsatMediaOverallDTO> filterVsatMediaOverall(VsatMediaOverallFilterDTO data) {
        try {
            ...
            if (!StringUtil.isNullOrEmpty(data.getFromTime()) && !StringUtil.isNullOrEmpty(data.getToTime())) {
                ...
                boolQueryBuilder.must(QueryBuilders.rangeQuery("eventTime").from(from.getTime()).to(to.getTime()));
            }
            ...
            if (data.getDirection() != null) {
                boolQueryBuilder.must(QueryBuilders.termQuery("direction", data.getDirection()));
            }
            ...
            // Sort
            nativeSearchQueryBuilder.withSort(SortBuilders.fieldSort("eventTime").order(SortOrder.DESC));
            ...
            List<VsatMediaOverallDTO> vsatMediaAnalyzedList = new ArrayList<>();
            ...
            vsatMediaAnalyzedList = searchHits.stream().map((s) -> enrichVsatMediaOverall(s.getContent(), s.getId())).collect(Collectors.toList());
    ...
    public List<VsatMediaOverallDTO> findByUuidList(List<String> vsatMediaUuidLst) {
        try {
            NativeSearchQueryBuilder nativeSearchQueryBuilder = new NativeSearchQueryBuilder();
            nativeSearchQueryBuilder.withPageable(PageRequest.of(0, 200));
            nativeSearchQueryBuilder.withTrackTotalHits(true);

            BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();

            if (vsatMediaUuidLst != null && !vsatMediaUuidLst.isEmpty()) {
                BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
                vsatMediaUuidLst.forEach((uuidKey) -> {
                    boolQuery.should(QueryBuilders.matchPhraseQuery("id", uuidKey));
                });
            ...
            return vsatMediaAnalyzedList;
        } catch (Exception ex) {
            LOGGER.error("ERROR find by uuid list: ", ex);
            ex.printStackTrace();
        }
    ...
    private VsatMediaOverallDTO enrichVsatMediaOverall(Object objectVsatMedia, String uuid) {
        VsatMediaOverallDTO vsatMediaAnalyzed = modelMapper.map(objectVsatMedia, VsatMediaOverallDTO.class);
